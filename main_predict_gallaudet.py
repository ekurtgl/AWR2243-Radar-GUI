...
layout = [[sg.Text('Radar User Interface', size=(50, 2), font=('courier', font_size))],
          ...
          [sg.Image(key='-IMAGE-', size=spect_size, background_color='white'),
           sg.VSep(),
           graph],
          [
          ...
          [sg.Checkbox('Teaching', default=False, key="teaching_check", size=(10, 1), font=('courier', font_size)),
           sg.Checkbox('Testing', default=False, key="testing_check", size=(10, 1), font=('courier', font_size))],
          [
           ...
           # sg.VSep(),
           sg.Button('Start Recording', key='train_start_record', button_color=('white', 'green'),
          ...
          [
           ...
           # sg.VSep(),
           sg.Button('Stop Recording', key='train_stop_record', button_color=('white', 'red'),
                     size=(18, 2), font=('courier', font_size)),
          ...
          [sg.Button('Predict on\nNew Data', key='pred_new', button_color=('black', 'yellow'),
                     size=(18, 2), font=('courier', font_size)),
...
while True:  # Event Loop
    ...
    if event == 'Setup Radar':
        ...
        window['train_stop_record_text'].update('Done!                          ')
        window.refresh()
    ...
    # elif event == '3. Micro-Doppler Signature':
        ...
        if values['teaching_check'] and not values['testing_check']:
            cmd = subprocess.Popen(start_cmd, cwd=cwd, shell=False, stdin=subprocess.PIPE, text=True,
                                   stdout=subprocess.PIPE, stderr=subprocess.PIPE)
            cmd.wait()
            # cmd.stdin.write("start_record cf.json")
            ...
            # event = '1. Start Recording'
            print('start testing record...')
            # window['-IMAGE-'].update('data/md.png', size=spect_size)
            # window['-VIDEO-'].update('data/rd.png')
            # window['md_text'].update('                               ')
            # window['rd_text'].update('')
        ...
        model, history = CNN_train(data, is_split, 3, 0.001, 1, 50,
                                   new_model_name)

    elif event == 'pred_new':
        if values['testing_check'] and not values['teaching_check']:
            ...
            window['pred_new_text'].update('Please check Testing and\nuncheck Teaching to make a prediction')
